#' Add a rewrite command to the bot.
#'
...
#' @param mss.write Message info is already imputed.
#' @param mss.previous Message to show previous info.
#' @return A 'BirdBotSettings' object.
#' @examples
#' Settings <- add.sampling(Settings, command="rewrite", db.name ="database", description="add info to each nest for today", sep="-", today= 0, type="Info", mss.filled=" info filled.", mss.tasks="The task for today was: ", mss.write="Please use command /nest instead of /rewrite to write new info.", mss.previous=" was previously filled as:")
#'
#' Settings <- add.sampling(Settings, command="rewrite-yesterday", db.name ="database", description="add info to each nest for yesterday", sep=".", today= -1, type="Info", mss.filled=" info filled.", mss.tasks="The task for yesterday was: ", mss.write="Info for not imputed", mss.previous=" was previously filled as:")
#'@export
add.rewrite <- function(x=Settings,
                         command="nest",
                         db.name ="database",
                         ...
                         mss.write="Please use command /rewrite instead of /nest to rewrite wrong info.",
  ...
  rewrite.f <- function(bot=x$Configuration$bot,update,response){
      ...
      if(nest.number %in% x$Locations$Nests$nests.id){
        if(update$message$from$id %in% x$Users$Allowed){
          ...
          if(!is.na(nest.today[,type])){
            ...
            bot$sendMessage(chat_id = update$message$chat$id, text = mss.write)
  ...
  x$Interaction$Commands[[command]]<- rewrite.f
